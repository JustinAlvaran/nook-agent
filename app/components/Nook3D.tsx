...
function RobotModel({ primary, secondary, faceGlow, outfit, accessory, motion }: Required<Pick<Props, "primary" | "secondary" | "faceGlow" | "outfit" | "accessory" | "motion">>) {
  ...
  useFrame(({ clock: sceneClock, pointer }, delta) => {
    ...
    const bob = reduced ? 0 : Math.sin(t * 2.1) * 0.035;
    root.current.position.y = bob;
    root.current.rotation.y = THREE.MathUtils.lerp(root.current.rotation.y, pointer.x * 0.11, 0.04);
    head.current.rotation.y = THREE.MathUtils.lerp(head.current.rotation.y, pointer.x * 0.2, 0.055);
    head.current.rotation.x = THREE.MathUtils.lerp(head.current.rotation.x, -pointer.y * 0.11, 0.055);
    ...
    if (motion === "walk") {
      ...
      root.current.position.x = reduced ? 0 : Math.sin(t * 18) * 0.025;
    } else {
  ...
  return <group ref={root} scale={1.05}>
    ...
    <mesh position={[-.34,-1.13,.05]} scale={[.34,.22,.44]}><sphereGeometry args={[1,24,16]}/><meshStandardMaterial color={secondary}/></mesh>
    <mesh position={[.34,-1.13,.05]} scale={[.34,.22,.44]}><sphereGeometry args={[1,24,16]}/><meshStandardMaterial color={secondary}/></mesh>
  </group>;
}
...
export function Nook3D({ name="Orbit", primary="#617fff", secondary="#9db0ff", faceGlow="#7debff", outfit="hoodie", accessory="star", motion="idle", message, draggable=true, compact=false, onPositionChange }: Nook3DProps) {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("nook-position");
  ...
  function pointerUp(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    window.localStorage.setItem("nook-position", JSON.stringify(offset)); onPositionChange?.(offset);
  }
  function moveBy(dx:number, dy:number) {
    const next={x:Math.max(-150,Math.min(150,offset.x+dx)),y:Math.max(-55,Math.min(70,offset.y+dy))}; setOffset(next); window.localStorage.setItem("nook-position",JSON.stringify(next)); onPositionChange?.(next);
  }
  ...
  return <div className={`nook3d ${compact ? "nook3d-compact" : ""} ${dragging ? "is-dragging" : ""}`} style={{ transform:`translate3d(${offset.x}px,${offset.y}px,0)` }}>
    ...
    <div className="nook3d-canvas" role="img" aria-label={`${name}, your customizable Nook companion. ${dragging ? "Being repositioned." : `Current motion: ${motion}.`}`} tabIndex={0} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onKeyDown={(event)=>{const step=event.shiftKey?24:8;if(event.key==="ArrowLeft")moveBy(-step,0);if(event.key==="ArrowRight")moveBy(step,0);if(event.key==="ArrowUp")moveBy(0,-step);if(event.key==="ArrowDown")moveBy(0,step)}}>
      {!webglFailed ? <Canvas orthographic camera={{ position:[0,.1,6], zoom:90 }} dpr={[1,1.5]} gl={{ alpha:true, antialias:true, powerPreference:"high-performance" }} onCreated={({gl})=>gl.domElement.addEventListener("webglcontextlost",()=>setWebglFailed(true),{once:true})}>
        ...
        <RobotModel primary={primary} secondary={secondary} faceGlow={faceGlow} outfit={outfit} accessory={accessory} motion={dragging?"drag":motion}/>
