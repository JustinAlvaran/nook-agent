...
begin
  ...
  if has_function_privilege('anon','public.nook_decide_simulated_approval(uuid,text,text)','execute') then raise exception 'approval RPC exposed to anon'; end if;
  if not has_function_privilege('authenticated','public.nook_create_planned_task(uuid,uuid,text,text,integer,jsonb,jsonb,jsonb)','execute') then raise exception 'planned task RPC unavailable'; end if;
